...
export interface QueueProps {
   ...
   * and `FifoThroughputLimit` is automatically set to `perMessageGroupId` on the
...
export class Queue extends QueueBase {
  ...
  private determineFifoProps(props: QueueProps): FifoProps {
    ...
    if (props.highThroughputFifoEnabled && !fifoQueue) {
      throw new ValidationError('highThroughputFifoEnabled can only be set for FIFO queues', this);
    }
