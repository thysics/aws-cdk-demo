...
export interface QueueProps {
  ...
  /**
   * Specifies whether to enable high throughput for a FIFO queue.
   *
   * When set to true, this automatically configures:
   * - `deduplicationScope` to `DeduplicationScope.MESSAGE_GROUP`
   * - `fifoThroughputLimit` to `FifoThroughputLimit.PER_MESSAGE_GROUP_ID`
   *
   * (Only applies to FIFO queues.)
   *
   * @default false
...
export class Queue extends QueueBase {
  ...
  private determineFifoProps(props: QueueProps): FifoProps {
    ...
    if (props.highThroughputFifoEnabled && !fifoQueue) {
      throw new ValidationError('High throughput FIFO can only be enabled for FIFO queues', this);
    }
