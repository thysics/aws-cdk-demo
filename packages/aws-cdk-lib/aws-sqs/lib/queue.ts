...
export interface QueueProps {
   ...
   *
   * When set to true, this automatically configures the queue for high throughput
   * by setting `deduplicationScope` to `DeduplicationScope.MESSAGE_GROUP` and
...
export class Queue extends QueueBase {
  ...
  private determineFifoProps(props: QueueProps): FifoProps {
    ...
    if (props.highThroughputFifoEnabled && !fifoQueue) {
      throw new ValidationError('highThroughputFifoEnabled can only be defined for FIFO queues', this);
    }
