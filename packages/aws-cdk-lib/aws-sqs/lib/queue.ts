...
export interface QueueProps {
  ...
  /**
   * Enable high throughput sending and receiving for a FIFO queue.
   *
   * When set to true, this automatically sets `DeduplicationScope` to `messageGroup`
   * and `FifoThroughputLimit` to `perMessageGroupId` on the underlying CloudFormation resource.
   *
   * Can only be used with FIFO queues (i.e., `fifo` must be true).
   *
   * @default false
   */
