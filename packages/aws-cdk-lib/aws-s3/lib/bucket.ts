...
export interface IntelligentTieringConfiguration {
  ...
  /**
   * A list of S3 Intelligent-Tiering access tier and days configurations.
   *
   * Each entry specifies an access tier and the number of days before
   * objects are moved to that tier.
   *
   * @default No additional tierings beyond archiveAccessTierTime and deepArchiveAccessTierTime
   */
  readonly tierings?: Tiering[];
}

/**
 * Represents a single tiering configuration for S3 Intelligent-Tiering.
 */
export interface Tiering {
  /**
   * The S3 Intelligent-Tiering access tier.
   *
   * For example, 'ARCHIVE_ACCESS' or 'DEEP_ARCHIVE_ACCESS'.
   */
  readonly accessTier: string;

  /**
   * The number of consecutive days of no access after which an object
   * will be eligible to be transitioned to the corresponding tier.
   */
...
export class Bucket extends BucketBase {
  ...
  private parseTieringConfig({ intelligentTieringConfigurations }: BucketProps): CfnBucket.IntelligentTieringConfigurationProperty[] | undefined {
    ...
    return intelligentTieringConfigurations.map(config => {
      ...
      if (config.tierings) {
        for (const t of config.tierings) {
          tierings.push({
