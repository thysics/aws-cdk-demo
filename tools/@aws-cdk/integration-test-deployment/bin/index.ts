...
import { parsePermissionsSnapshotArgs } from '../lib/permissions-snapshot-integration';
...
// Parse permissions snapshot options from CLI arguments
const permissionsSnapshot = parsePermissionsSnapshotArgs(process.argv.slice(2));

// Environment variable overrides
if (process.env.SKIP_PERMISSIONS_SNAPSHOT === 'true') {
  permissionsSnapshot.enabled = false;
}
if (process.env.UPDATE_PERMISSIONS_SNAPSHOT === 'true') {
  permissionsSnapshot.updateSnapshots = true;
}
...
deployIntegTests({ atmosphereRoleArn, endpoint, pool, batchSize, permissionsSnapshot }).catch((e) => {
  console.error(e);
  process.exitCode = 1;
});
